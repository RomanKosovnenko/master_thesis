...
\paragraph{The Request} represents a user requirement, which specifies which functionality should be used to execute parameters and requirements. Requests contain their functional requirements by referring to a target software component, limitations on non-functional requirements (e.g., quality).\\
...
\todo{this phrase is hardly readable.. Could you rephrase it somehow?}There are relations between component implementation and another software component that this implementation is requiring, between component implementation and HW resource that this implementation is required.
...
\begin{itemize}
	...
	\item fixed branching factor of two. \todo{for the first-time-reader it is unclear, which "branching factor" do you mean. I would add a short clarification in parentheses}
\end{itemize}
...
The solution of the MQuAT Problem could be represented as a tree structure. An example of the Solution shown in Figure ~\ref{fig:SolutionModel}. It contains a list of assignments. Each assignment selects one implementation of the required component and maps it on the resources~\cite{gotz18}.
...
The mutation is an operator of an Evolution Algorithm that changes a single gene in a chromosome. Similarly to the crossover, a mutation has parameter \texttt{Mutation rate}. It describes the probability of mutation.
...
If genotype can not be described as a vector, then it is needed to implement:
