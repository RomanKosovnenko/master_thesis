\chapter{Background}
This thesis \rev{is} based on several kinds of research of SAS\todoD{not introduced}, auto-tuning, evolutionary algorithms, and software optimization. This chapter summarizes the important aspects and details of approaches and optimization \rev{techniques} used in the succeeding chapters. 
\todoD{in general this part should be rephrased when the intro is ready.} 

\section{Multi\rev{-}Quality Auto\rev{-Tuning~}(MQuAT)}
...
It is based on the design-time part, which represents a new development method for self-optimizing systems and runtime parts, which concerns operation principles, namely,  novel techniques to runtime self-optimization~\cite{gotz13}.\todoD{rephrase}

\subsection{Design principles of MQuAT}
MQuAT presented a new method of developing self-optimized software. In which software is proposed to be constructed from components with specifically defined limits~\todo{DP: what limits?}. In addition, components are intended to comprise multiple implementations, each providing the same but differing functionality in their non-functional behavior\todoD{the same functionality but differing in their non-functional behavior}. Therefore, the design principle is the critical factor for runtime optimization because when there is a different configuration, optimization can be done, and the optimal or almost optimal configuration can be selected~\todoD{rephrase}. To compare different implementations of the software component, they need to be specified with their non-functional properties (NFP)~\cite{gotz13}.
A new meta-architecture of the self-optimizing software system was created \rev{to} highlight this fundamental principle. It called the cool component model~\cite{gotz10}.\todoD{to highlight? what do the last two sentences tell us?}
A specialty\todo{DP:typo?} of MQuAT is the application of QoS contracts to cover non-functional implementation behavior as well as the interrelationships of different components between NFPs. Contracts naturally describe the relationship between provisions and requirements.
\todoD{Here I stop reviewing MQuAT subsection. Rethink it and reorganize. First, write the research question in Chapter 1.}
